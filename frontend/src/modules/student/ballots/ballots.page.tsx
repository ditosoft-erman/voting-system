      ...
      <div className="p-6 bg-white">
        ...
        <div className="mb-4">
          ...
          {ballot.candidatesPresident.map((candidate, index) => (
            <div key={index} className="flex items-center p-2 bg-gray-200 rounded-md mb-2">
              <img src={candidate.image} alt={candidate.name} className="w-10 h-10 mr-2 rounded-full" />
              <span>{candidate.name}</span>
              <input type="radio" name="president" className="ml-auto" checked={candidate.selected} />
            </div>
          ))}
        </div>
        {/* Vice-President */}
        <div className="mb-4">
          ...
          {ballot.candidatesVPresident.map((candidate, index) => (
            <div key={index} className="flex items-center p-2 bg-gray-200 rounded-md mb-2">
              <img src={candidate.image} alt={candidate.name} className="w-10 h-10 mr-2 rounded-full" />
              <span>{candidate.name}</span>
              <input type="radio" name="vice-president" className="ml-auto" checked={candidate.selected} />
            </div>
          ))}
        </div>
        {/* Secretary */}
        <div className="mb-4">
          ...
          {ballot.candidatesSecretary.map((candidate, index) => (
            <div key={index} className="flex items-center p-2 bg-gray-200 rounded-md mb-2">
              <img src={candidate.image} alt={candidate.name} className="w-10 h-10 mr-2 rounded-full" />
              <span>{candidate.name}</span>
              <input type="radio" name="secretary" className="ml-auto" checked={candidate.selected} />
            </div>
          ))}
        </div>
        <button className="mt-6 px-4 py-2 bg-blue-900 text-white rounded-md" onClick={onClose}>
          Close
        </button>
      </div>
    </Modal>
  ...
  const ballots: Ballot[] = [
    {
      ...
      candidatesPresident: [
        { name: 'Emma Carter', image: 'https://via.placeholder.com/40', selected: true },
        { name: 'Ava Mitchell', image: 'https://via.placeholder.com/40', selected: false }
      ],
      candidatesVPresident: [
        { name: 'Emma Carter', image: 'https://via.placeholder.com/40', selected: false },
        { name: 'Ava Mitchell', image: 'https://via.placeholder.com/40', selected: true }
      ],
      candidatesSecretary: [
        { name: 'Emma Carter', image: 'https://via.placeholder.com/40', selected: false },
        { name: 'Ava Mitchell', image: 'https://via.placeholder.com/40', selected: true }
    ...
    <div className="min-h-screen bg-gray-100">
      ...
      <div className="p-6">
        <div className="flex justify-between items-center mb-4">
          ...
          <div className="flex">
            <input type="text" placeholder="Search" className="border border-gray-300 rounded-l-md px-2 py-1" />
              ...
              {ballots.map((ballot, index) => (
                <tr key={index} className="border">
                  ...
                  <td className="border px-4 py-2 flex justify-center space-x-2">
                    <button className="bg-yellow-400 px-2 py-1 rounded" onClick={() => openModal(ballot)}>
                      View
                    </button>
      ...
      {/* Ballot Modal */}
      <BallotDetailModal isOpen={modalIsOpen} onClose={closeModal} ballot={selectedBallot} />
    </div>
  );
};
